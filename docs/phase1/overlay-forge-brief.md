...
7. 不把上游 First-Light 的 Overlay CI 混进本仓。源仓仍 pin `overlay-v1.0.0`、套件 `armed` / `draft`；本仓保持 `overlay-v2.0.0` 与 `active`/`blocked`。计数与不收清单见 [OF-07](./overlay-forge-issues.md)。
8. 不为半锁叶子再发明第二套 ID。
